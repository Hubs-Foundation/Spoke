...
export default class KitPieceNode extends EditorNodeMixin(Model) {
  ...
  static async deserialize(editor, json, loadAsync) {
    ...
    loadAsync(
      (async () => {
        const { src, nodeId } = json.components.find(c => c.name === "kit-piece").props;

        await node.load(src, nodeId);
  ...
  loadGLTF(src, nodeId) {
    return this.editor.gltfCache.getNode(src, nodeId);
  }
  ...
  async load(src, nodeId) {
    ...
    if (nextSrc === this._canonicalUrl && nextNodeId === this.nodeId) {
      return;
    }
    ...
    try {
      ...
      await super.load(accessibleUrl, nodeId);
