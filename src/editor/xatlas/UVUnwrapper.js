...
import xatlasWasmUrl from "xatlas-web/dist/xatlas-web.wasm";
...
export default class UVUnwrapper {
  static async load() {
    xatlas = XAtlas({
      locateFile(path) {
        if (path.endsWith(".wasm")) {
          return xatlasWasmUrl;
        }
        return path;
      }
    });
  ...
  unwrap(mesh) {
    ...
    if (statusCode !== AddMeshStatus.Success) {
      throw new Error(`Error adding mesh. Status code ${statusCode}`);
    }
